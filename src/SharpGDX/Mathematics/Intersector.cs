 ...
 * @author Nathan Sweet */
    public sealed class Intersector
    {
        ...
        private readonly static Vector3 v0 = new Vector3();
        private readonly static Vector3 v1 = new Vector3();
        private readonly static Vector3 v2 = new Vector3();
        private readonly static FloatArray floatArray = new FloatArray();
        private readonly static FloatArray floatArray2 = new FloatArray();
        ...
        /** Returns true if the given point is inside the triangle. */
        public static bool isPointInTriangle(float px, float py, float ax, float ay, float bx, float by, float cx,
            float cy)
        {
            float px1 = px - ax;
            float py1 = py - ay;
        ...
        {
            return (int)Math
                .Sign((linePoint2X - linePoint1X) * (pointY - linePoint1Y) -
        ...
        public static bool isPointInPolygon(Array<Vector2> polygon, Vector2 point)
        {
        ...
        private readonly static Vector2 ip = new Vector2();
        private readonly static Vector2 ep1 = new Vector2();
        private readonly static Vector2 ep2 = new Vector2();
        private readonly static Vector2 s = new Vector2();
        private readonly static Vector2 e = new Vector2();
        ...
        static public bool intersectPolygons(FloatArray polygon1, FloatArray polygon2)
        {
        ...
        static public bool intersectPolygonEdges(FloatArray polygon1, FloatArray polygon2)
        {
        ...
        public static bool intersectFrustumBounds(Frustum frustum, BoundingBox bounds)
        {
            bool boundsIntersectsFrustum = frustum.pointInFrustum(bounds.getCorner000(tmp))
            ...
            foreach (Vector3 point in frustum.planePoints)
            {
        ...
        public static bool intersectFrustumBounds(Frustum frustum, OrientedBoundingBox obb)
        {

            bool boundsIntersectsFrustum = false;

            foreach (Vector3 v in obb.getVertices())
            {
        ...
        private static readonly Plane p = new Plane(new Vector3(), 0);
        private static readonly Vector3 i = new Vector3();
        ...
        private static readonly Vector3 start = new Vector3();
        ...
        public static bool intersectRaySphere(Ray ray, Vector3 center, float radius, Vector3 intersection)
        {
            ...
            if (len < 0.0f) // behind the ray
                return false;
            float dst2 = center.dst2(ray.origin.x + ray.direction.x * len, ray.origin.y + ray.direction.y * len,
        ...
        static public bool intersectRayBoundsFast(Ray ray, BoundingBox box)
        {
         ...
         * @return Whether the ray and the bounding box intersect. */
        static public bool intersectRayBoundsFast(Ray ray, Vector3 center, Vector3 dimensions)
        {
        ...
        static public bool intersectRayOrientedBoundsFast(Ray ray, OrientedBoundingBox obb)
        {
         ...
         *
         * @return Whether the ray and the oriented bounding box intersect. */
        static public bool intersectRayOrientedBoundsFast(Ray ray, BoundingBox bounds, Matrix4 transform)
        {
         ...
         * @param intersection The intersection point (optional)
         * @return Whether an intersection is present. */
        static public bool intersectRayOrientedBounds(Ray ray, OrientedBoundingBox obb, Vector3 intersection)
        {
         ...
         * @param intersection The intersection point (optional)
         * @return Whether an intersection is present. */
        static public bool intersectRayOrientedBounds(Ray ray, BoundingBox bounds, Matrix4 transform,
            Vector3 intersection)
        {
            float tMin = 0.0f;
         ...
         * @param intersection The nearest intersection point (optional)
         * @return Whether the ray and the triangles intersect. */
        public static bool intersectRayTriangles(Ray ray, float[] vertices, short[] indices, int vertexSize,
            Vector3 intersection)
        {
            float min_dist = float.MaxValue;
            bool hit = false;
        ...
        public static bool intersectRayTriangles(Ray ray, List<Vector3> triangles, Vector3 intersection)
        {
            ...
            if (triangles.Count % 3 != 0) throw new RuntimeException("triangle list size is not a multiple of 3");

            for (int i = 0; i < triangles.Count; i += 3)
            {
                bool result = intersectRayTriangle(ray, triangles[i], triangles[i + 1], triangles[i + 2], tmp);
        ...
        {
            return intersectBoundsPlaneFast(box.getCenter(tmp1), box.getDimensions(tmp2).scl(0.5f), plane.normal,
                plane.d);
        }
         ...
         * @return Whether the two lines intersect */
        public static bool intersectLines(float x1, float y1, float x2, float y2, float x3, float y3, float x4,
            float y4,
        ...
        public static bool intersectSegmentRectangle(Vector2 start, Vector2 end, Rectangle rectangle)
        {
        ...
        public static bool intersectSegments(Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4, Vector2 intersection)
        {
            return intersectSegments(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, p4.x, p4.y, intersection);
        }

        /** @param intersection May be null. */
        public static bool intersectSegments(float x1, float y1, float x2, float y2, float x3, float y3, float x4,
            float y4,
            Vector2 intersection)
        {
            float d = (y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1);
         ...
         * @param mtv A Minimum Translation Vector to fill in the case of a collision, or null (optional).
         * @return Whether polygons overlap. */
        public static bool overlapConvexPolygons(float[] verts1, int offset1, int count1, float[] verts2, int offset2,
            int count2,
            MinimumTranslationVector mtv)
        {
            bool overlaps;
        ...
        private static bool overlapsOnAxisOfShape(float[] verts1, int offset1, int count1, float[] verts2, int offset2,
            int count2,
            MinimumTranslationVector mtv, bool shapesShifted)
        {
        ...
        private static void splitEdge(float[] vertices, int s, int e, int stride, Plane plane, float[] split,
            int offset)
        {
        ...
        public class SplitTriangle
        {
            ...
            {
                return "SplitTriangle [front=" + Arrays.toString(front) + ", back=" + Arrays.toString(back) +
                       ", numFront=" + numFront
                       + ", numBack=" + numBack + ", total=" + total + "]";
            }

            internal void setSide(bool front)
            {
                frontCurrent = front;
            }

            internal bool getSide()
            {
                return frontCurrent;
            }

            internal void add(float[] vertex, int offset, int stride)
            {
                if (frontCurrent)
                {
        ...
        public static bool hasOverlap(Vector3[] axes, Vector3[] aVertices, Vector3[] bVertices)
        {
            foreach (Vector3 axis in axes)
            {
                ...
                float maxA = -float.MaxValue;
                // project shape a on axis
                foreach (Vector3 aVertex in aVertices)
                {
                ...
                float maxB = -float.MaxValue;
                // project shape b on axis
                foreach (Vector3 bVertex in bVertices)
                {
