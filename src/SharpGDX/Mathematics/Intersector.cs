...
public sealed class Intersector {
	...
	/** Returns true if the given point is inside the triangle. */
	public static bool isPointInTriangle (float px, float py, float ax, float ay, float bx, float by, float cx, float cy) {
		float px1 = px - ax;
		float py1 = py - ay;
	...
	public static int pointLineSide (float linePoint1X, float linePoint1Y, float linePoint2X, float linePoint2Y, float pointX,
		float pointY) {
		return (int)Math
			.Sign((linePoint2X - linePoint1X) * (pointY - linePoint1Y) - (linePoint2Y - linePoint1Y) * (pointX - linePoint1X));
	}
	 ...
	 * @param intersection The intersection point (optional)
	 * @return Whether an intersection is present. */
	static public bool intersectRayOrientedBounds (Ray ray, BoundingBox bounds, Matrix4 transform, Vector3 intersection) {
		float tMin = 0.0f;
	 ...
	 * @return Whether the two lines intersect */
	public static bool intersectLines (float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4,
	...
	public static bool intersectSegments (float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4,
		Vector2 intersection) {
		float d = (y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1);
		if (d == 0) return false;
	 ...
	 * @param mtv A Minimum Translation Vector to fill in the case of a collision, or null (optional).
	 * @return Whether polygons overlap. */
	public static bool overlapConvexPolygons (float[] verts1, int offset1, int count1, float[] verts2, int offset2, int count2,
	...
	private static bool overlapsOnAxisOfShape (float[] verts1, int offset1, int count1, float[] verts2, int offset2, int count2,
	...
	public static void splitTriangle (float[] triangle, Plane plane, SplitTriangle split) {
		...
		bool r2 = plane.testPoint(triangle[0 + stride], triangle[1 + stride], triangle[2 + stride]) == Plane.PlaneSide.Back;
