 ...
 * @author Tommy Ettinger */
	public class LongMap<V> : IEnumerable<LongMap<V>.Entry> {
	public int size;
	...
	public LongMap(int initialCapacity, float loadFactor)
	{
		...
		// TODO: Is this the right generic?
		int tableSize = ObjectSet<long>.tableSize(initialCapacity, loadFactor);
	...
	public void shrink(int maximumCapacity)
	{
		...
		int tableSize = ObjectSet<long>.tableSize(maximumCapacity, loadFactor);
	 ...
	 * @param identity If true, uses == to compare the specified value with values in the map. If false, uses
	 *           {@link #equals(Object)}. */
	public long findKey(Object? value, bool identity, long notFound)
	{
		V[] valueTable = this.valueTable;
		if (value == null)
		{
	...
	{
		// TODO: Is this the correct generic?
			int tableSize = ObjectSet<long>.tableSize(size + additionalCapacity, loadFactor);
		if (keyTable.Length < tableSize) resize(tableSize);
	}
	...
	public override bool Equals(Object? obj)
	{
		...
		for (int i = 0, n = keyTable.Length; i < n; i++)
		{
			...
			if (key != 0)
			{
				...
				if (value == null)
				{
					if (other.get(key, ObjectMap<long, V>.dummy) != null) return false;
				}
				else
				{
	...
	public bool equalsIdentity(Object? obj)
	{
		...
		{
			long key = keyTable[i];
			if (key != 0 && !ReferenceEquals(valueTable[i] , other.get(key, ObjectMap<long, V>.dummy))) return false;
		}
		return true;
	}
	...
	public class MapIterator
	{
		...
		internal readonly LongMap<V> map;
