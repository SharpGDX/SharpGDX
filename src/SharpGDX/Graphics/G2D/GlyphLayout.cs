 ...
 * @author Thomas Creutzenberg */
    public class GlyphLayout : IPoolable
    {
            ...
            bool wrap, String? truncate)
        {
            ...
            BitmapFont.Glyph
                lastGlyph = null; // Last glyph of the previous run on the same line, used for kerning between runs.
            int runStart = start;
            ...
            while (true)
            {
...

                {
                    ...
                    if (newline || isLastRun)
                    {
                        ...
                        float runWidth =
                            lineRun.xAdvances.first() + lineRun.xAdvances.get(1); // At least the first glyph will fit.
                        for (int i = 2; i < lineRun.xAdvances.size; i++)
                        {
                            ...
                            runWidth = lineRun.xAdvances.first() +
                                       lineRun.xAdvances.get(1); // At least the first glyph will fit.
                            i = 1;
                        }
                    }
                }
        ...
        private void calculateWidths(BitmapFont.BitmapFontData fontData)
        {
            ...
            for (int i = 0, n = runs.size; i < n; i++)
            {
                ...
                float runWidth = run.x + xAdvances[0],
                    max = 0; // run.x is needed to ensure floats are rounded same as above.
                ...
                for (int ii = 0, nn = run.glyphs.size; ii < nn;)
                {
                    ...
                    float glyphWidth = getGlyphWidth(glyph, fontData);
                    max = Math.Max(max,
                        runWidth + glyphWidth); // A glyph can extend past the right edge of subsequent glyphs.
                    ii++;
        ...
        private GlyphRun wrap(BitmapFont.BitmapFontData fontData, GlyphRun first, int wrapIndex)
        {
            ...
            if (secondStart < glyphCount)
            {
                ...
                if (fontData.markupEnabled && droppedGlyphCount > 0)
                {
                    ...
                    for (int i = colors.size - 2; i >= 2; i -= 2)
                    {
        ...
        {
            return (glyph.fixedWidth ? glyph.xadvance : glyph.width + glyph.xoffset) * fontData.scaleX -
                   fontData.padRight;
        }
