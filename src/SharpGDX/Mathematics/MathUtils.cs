 ...
 * @author Nathan Sweet */
	public sealed class MathUtils
	{
		...
		static public readonly float nanoToSec = 1 / 1000000000f;

		// ---
		static public readonly float FLOAT_ROUNDING_ERROR = 0.000001f; // 32 bits
		static public readonly float PI = (float)Math.PI;
		static public readonly float PI2 = PI * 2;
		static public readonly float HALF_PI = PI / 2;

		static public readonly float E = (float)Math.E;

		static private readonly int SIN_BITS = 14; // 16KB. Adjust for accuracy.
		static private readonly int SIN_MASK = ~(-1 << SIN_BITS);
		static private readonly int SIN_COUNT = SIN_MASK + 1;

		static private readonly float radFull = PI2;
		static private readonly float degFull = 360;
		static private readonly float radToIndex = SIN_COUNT / radFull;
		static private readonly float degToIndex = SIN_COUNT / degFull;

		/** multiply by this to convert from radians to degrees */
		static public readonly float radiansToDegrees = 180f / PI;
		static public readonly float radDeg = radiansToDegrees;
		/** multiply by this to convert from degrees to radians */
		static public readonly float degreesToRadians = PI / 180;
		static public readonly float degRad = degreesToRadians;
	...
	public static float tan(float radians)
	{
		...
		radians -= 0.5f;
		radians *= PI;
		float x2 = radians * radians, x4 = x2 * x2;
	...
	public static float tanDeg(float degrees)
	{
		...
		degrees -= 0.5f;
		degrees *= PI;
		 float x2 = degrees * degrees, x4 = x2 * x2;
	...
	public static float atanUnchecked(double i)
	{
		...
		double c9 = c7 * c2;
		double c11 = c9 * c2;
		return (float)(Math.Sign(i) * ((Math.PI * 0.25)
	...
	public static double atanUncheckedDeg(double i)
	{
		...
		double c9 = c7 * c2;
		double c11 = c9 * c2;
		return (Math.Sign(i) * (45.0 + (57.2944766070562 * c - 19.05792099799635 * c3 + 11.089223410359068 * c5
	...
	public static float atan(float i)
	{
		...
		double n = Math.Min(Math.Abs(i), double.MaxValue);
		// c uses the "equally-good" formulation that permits n to be from 0 to almost infinity.
		double c = (n - 1.0) / (n + 1.0);
		// The approximation needs 6 odd powers of c.
		double c2 = c * c;
		...
		double c9 = c7 * c2;
		double c11 = c9 * c2;
		return Math.Sign(i) * (float)((Math.PI * 0.25)
			+ (0.99997726 * c - 0.33262347 * c3 + 0.19354346 * c5 - 0.11643287 * c7 + 0.05265332 * c9 - 0.0117212 * c11));
	}
	...
	public static float atanDeg(float i)
	{
		...
		double c9 = c7 * c2;
		double c11 = c9 * c2;
		return (float)(Math.Sign(i) * (45.0 + (57.2944766070562 * c - 19.05792099799635 * c3 + 11.089223410359068 * c5
			- 6.6711120475953765 * c7 + 3.016813013351768 * c9 - 0.6715752908287405 * c11)));
	}

	// ---

	static public RandomXS128 _random = new RandomXS128();
	...
	static private readonly int BIG_ENOUGH_INT = 16 * 1024;
	static private readonly double BIG_ENOUGH_FLOOR = BIG_ENOUGH_INT;
	static private readonly double CEIL = 0.9999999;
	static private readonly double BIG_ENOUGH_CEIL = 16384.999999999996;
	static private readonly double BIG_ENOUGH_ROUND = BIG_ENOUGH_INT + 0.5f;
