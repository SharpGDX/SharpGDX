    ...
    public override void draw(IBatch batch, float parentAlpha)
    {
        ...
        if (displayText.Length == 0)
        {
            if ((!focused || disabled) && messageText != null)
            {
                ...
                drawMessageText(batch, messageFont, x + bgLeftWidth, y + textY + yOffset,
    ...
    public class TextFieldClickListener : ClickListener
    {
        ...
        public override bool keyTyped(InputEvent @event, char character)
        {
            ...
            else
            {
                ...
                bool add = enter
                    ? _textField.writeEnters
                    : (!_textField.onlyFontChars || _textField.style.font.getData().hasGlyph(character));
                ...
                if (add || remove)
                {
                    ...
                    if (remove)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _textField.text = _textField.text.Substring(0, _textField.cursor - 1) +
                                                  _textField.text.Substring(_textField.cursor--);
                                _textField.renderOffset = 0;
                            }

                            if (delete && _textField.cursor < _textField.text.Length)
                            {
                                _textField.text = _textField.text.Substring(0, _textField.cursor) +
                                                  _textField.text.Substring(_textField.cursor + 1);
                            }
                        }
                    }
                    ...
                    if (add && !remove)
                    {
                        ...
                        if (!enter && _textField.filter != null && !_textField.filter.acceptChar(_textField, character))
                            return true;
                        if (!_textField.withinMaxLength(_textField.text.Length - (_textField.hasSelection
                                ? Math.Abs(_textField.cursor - _textField.selectionStart)
                                : 0))) return true;
