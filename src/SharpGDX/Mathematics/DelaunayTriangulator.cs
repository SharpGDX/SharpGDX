...
public class DelaunayTriangulator {
	...
	private readonly IntArray quicksortStack = new IntArray();
	private float[] sortedPoints;
	private readonly ShortArray triangles = new ShortArray(false, 16);
	private readonly ShortArray _originalIndices = new ShortArray(false, 0);
	private readonly IntArray edges = new IntArray();
	private readonly BooleanArray complete = new BooleanArray(false, 16);
	private readonly float[] superTriangle = new float[6];
	private readonly Vector2 centroid = new Vector2();

	/** @see #computeTriangles(float[], int, int, boolean) */
	public ShortArray computeTriangles (FloatArray points, bool sorted) {
		return computeTriangles(points.items, 0, points.size, sorted);
	}

	/** @see #computeTriangles(float[], int, int, boolean) */
	public ShortArray computeTriangles (float[] polygon, bool sorted) {
	...
	public ShortArray computeTriangles(float[] points, int offset, int count, bool sorted)
	{
		...
		for (int pointIndex = offset; pointIndex < end; pointIndex += 2)
		{
			...
			bool[] completeArray = complete.items;
