...
public class Mesh : Disposable {
	...
	public Mesh (bool staticVertices, bool staticIndices, int maxVertices, int maxIndices, VertexAttributes attributes) {
	...
	public int getVertexSize () {
		return _vertices.getAttributes().vertexSize;
	}
	...
	public void render (ShaderProgram shader, int primitiveType, int offset, int count, bool autoBind) {
		...
		if (isVertexArray) {
			...
			int numInstances = 0;
			if (_isInstanced) numInstances = instances.getNumInstances();
	...
	public VertexAttributes getVertexAttributes () {
		return _vertices.getAttributes();
	}
	...
	public FloatBuffer getVerticesBuffer (bool forWriting) {
		return _vertices.getBuffer(forWriting);
	}
	...
	public void calculateBoundingBox (BoundingBox bbox) {
		...
		bbox.inf();
		 ...
		 int vertexSize = _vertices.getAttributes().vertexSize / 4;
		int idx = offset;
	...
	public BoundingBox extendBoundingBox ( BoundingBox @out, int offset, int count,  Matrix4 transform) {
		...
		if (offset < 0 || count < 1 || offset + count > max)
		 ...
		 FloatBuffer verts = _vertices.getBuffer(false);
		 ShortBuffer index = indices.getBuffer(false);
		 VertexAttribute posAttrib = getVertexAttribute(VertexAttributes.Usage.Position);
	...
	public float calculateRadiusSquared ( float centerX,  float centerY,  float centerZ, int offset, int count,
		 Matrix4 transform) {
		int numIndices = getNumIndices();
	...
	public ShortBuffer getIndicesBuffer (bool forWriting) {
	...
	public static void transform ( Matrix4 matrix,  float[] vertices, int vertexSize, int offset, int dimensions,
		int start, int count) {
	 ...
	 * @param start the vertex to start with
	 * @param count the amount of vertices to transform */
	public static void transformUV ( Matrix3 matrix,  float[] vertices, int vertexSize, int offset, int start,
		int count) {
		if (start < 0 || count < 1 || ((start + count) * vertexSize) > vertices.Length) throw new IndexOutOfBoundsException(
			"start = " + start + ", count = " + count + ", vertexSize = " + vertexSize + ", length = " + vertices.Length);
	...
	public Mesh copy (bool isStatic, bool removeDuplicates,  int[] usage) {
