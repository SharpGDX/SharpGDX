    ...
    private class WindowInputListener : InputListener
    {
        ...
        private void updateEdge(float x, float y)
        {
            ...
            float padTop = _window.getPadTop(),
                padLeft = _window.getPadLeft(),
                padBottom = _window.getPadBottom(),
                padRight = _window.getPadRight();
            ...
            if (_window._isMovable && _window.edge == 0 && y <= height && y >= height - padTop && x >= left &&
                x <= right) _window.edge = MOVE;
        }
        ...
        public override void touchDragged(InputEvent @event, float x, float y, int pointer)
        {
            ...
            _window.setBounds((float)Math.Round(windowX), (float)Math.Round(windowY), (float)Math.Round(width),
    ...
    public override void draw(IBatch batch, float parentAlpha)
    {
        ...
        if (stage != null)
        {
            ...
            if (style.stageBackground != null)
            {
                ...
                drawStageBackground(batch, parentAlpha, getX() + tmpPosition.x, getY() + tmpPosition.y,
                    getX() + tmpSize.x,
                    getY() + tmpSize.y);
            }
        }
