using System;
using SharpGDX.Graphics.G3D.Utils.ShapeBuilders;
...
using VertexInfo = SharpGDX.Graphics.G3D.Utils.MeshPartBuilder.VertexInfo;
 ...
 * {@link #end()}.
 * @author Xoppa */
public class MeshBuilder : MeshPartBuilder {
	/** maximum number of vertices mesh builder can hold (64k) */
	public static readonly int MAX_VERTICES = 1 << 16;
	/** highest index mesh builder can get (64k - 1) */
	public static readonly int MAX_INDEX = MAX_VERTICES - 1;

	private readonly static ShortArray tmpIndices = new ShortArray();
	private readonly static FloatArray tmpVertices = new FloatArray();

	private readonly MeshPartBuilder.VertexInfo vertTmp1 = new MeshPartBuilder.VertexInfo();
	private readonly MeshPartBuilder.VertexInfo vertTmp2 = new MeshPartBuilder.VertexInfo();
	private readonly MeshPartBuilder.VertexInfo vertTmp3 = new MeshPartBuilder.VertexInfo();
	private readonly MeshPartBuilder.VertexInfo vertTmp4 = new MeshPartBuilder.VertexInfo();

	private readonly Color tempC1 = new Color();
	...
	private bool hasUVTransform = false;
	private float[] _vertex;

	private bool vertexTransformationEnabled = false;
	private readonly Matrix4 positionTransform = new Matrix4();
	private readonly Matrix3 normalTransform = new Matrix3();
	...
	public void begin ( VertexAttributes attributes, int primitiveType) {
		...
		if (this._vertex == null || this._vertex.Length < stride) this._vertex = new float[stride];
	...
	private void endpart () {
		if (_part != null) {
			bounds.getCenter(_part.center);
			bounds.getDimensions(_part.halfExtents).scl(0.5f);
            _part.radius = _part.halfExtents.len();
			bounds.inf();
            _part.offset = istart;
            _part.size = indices.size - istart;
			istart = indices.size;
            _part = null;
		}
	}
	...
	public MeshPart part ( String id,  int primitiveType, MeshPart meshPart) {
        ...
        _part = meshPart;
        _part.id = id;
		this.primitiveType = _part.primitiveType = primitiveType;
		parts.Add(_part);
	...
	public void getVertices (float[] @out, int destOffset) {
		...
		Array.Copy(vertices.items, 0, @out, destOffset, vertices.size);
	}
	...
	public void getIndices (short[] @out, int destOffset) {
		...
		if ((destOffset < 0) || (destOffset > @out.Length - indices.size))
			throw new GdxRuntimeException("Array too small or offset out of range");
		Array.Copy(indices.items, 0, @out, destOffset, indices.size);
	}
	...
	public bool isVertexTransformationEnabled () {
		return vertexTransformationEnabled;
	}

	public void setVertexTransformationEnabled (bool enabled) {
	...
	public void ensureTriangleIndices (int numTriangles) {
		if (primitiveType == IGL20.GL_LINES)
			ensureIndices(6 * numTriangles);
		else if (primitiveType == IGL20.GL_TRIANGLES || primitiveType == IGL20.GL_POINTS)
	...
	public void ensureRectangleIndices (int numRectangles) {
		if (primitiveType == IGL20.GL_POINTS)
			ensureIndices(4 * numRectangles);
		else if (primitiveType == IGL20.GL_LINES)
	...
	private readonly static Vector3 vTmp = new Vector3();

	private  static void transformPosition ( float[] values,  int offset,  int size, Matrix4 transform) {
		if (size > 2) {
	...
	private static void transformNormal (float[] values, int offset, int size, Matrix3 transform) {
		if (size > 2) {
	...
	private void addVertex (float[] values, int offset) {
		 int o = vertices.size;
		...
		if (vertexTransformationEnabled) {
		 ...
		 float x = vertices.items[o + posOffset];
		 float y = (posSize > 1) ? vertices.items[o + posOffset + 1] : 0f;
		 float z = (posSize > 2) ? vertices.items[o + posOffset + 2] : 0f;
		bounds.ext(x, y, z);
	...
	private readonly Vector3 tmpNormal = new Vector3();

	public short vertex (Vector3 pos, Vector3 nor, Color col, Vector2 uv) {
		if (vindex > MAX_INDEX) throw new GdxRuntimeException("Too many vertices used");

        _vertex[posOffset] = pos.x;
		if (posSize > 1) _vertex[posOffset + 1] = pos.y;
		if (posSize > 2) _vertex[posOffset + 2] = pos.z;

		if (norOffset >= 0) {
			if (nor == null) nor = tmpNormal.Set(pos).nor();
            _vertex[norOffset] = nor.x;
            _vertex[norOffset + 1] = nor.y;
            _vertex[norOffset + 2] = nor.z;
		}

		if (colOffset >= 0) {
			if (col == null) col = Color.White;
            _vertex[colOffset] = col.R;
            _vertex[colOffset + 1] = col.G;
            _vertex[colOffset + 2] = col.B;
			if (colSize > 3) _vertex[colOffset + 3] = col.A;
		} else if (cpOffset > 0) {
			if (col == null) col = Color.White;
            _vertex[cpOffset] = col.ToFloatBits(); // FIXME cache packed color?
		}

		if (uv != null && uvOffset >= 0) {
            _vertex[uvOffset] = uv.x;
            _vertex[uvOffset + 1] = uv.y;
		}

		addVertex(_vertex, 0);
		return (short)_lastIndex;
	}
	...
	public void line (short index1, short index2) {
		if (primitiveType != IGL20.GL_LINES) throw new GdxRuntimeException("Incorrect primitive type");
		index(index1, index2);
	}
	...
	public void triangle (short index1, short index2, short index3) {
		if (primitiveType == IGL20.GL_TRIANGLES || primitiveType == IGL20.GL_POINTS) {
	...
	public void rect (short corner00, short corner10, short corner11, short corner01) {
		if (primitiveType == IGL20.GL_TRIANGLES) {
		...
		} else if (primitiveType ==IGL20.GL_POINTS) {
	...
	public void addMesh (float[] vertices, short[] indices, int indexOffset, int numIndices) {
		...
		ensureIndices(numIndices);
		 int numVertices = vertices.Length / stride;
		...
		for (int i = 0; i < numIndices; i++) {
			...
			if (didx < 0) {
				...
				indicesMap.put(sidx, didx = _lastIndex);
			}
			index((short)didx);
		}
	}
	...
	[Obsolete]
	public void circle (float radius, int divisions,  Vector3 center,  Vector3 normal,  Vector3 tangent,
		 Vector3 binormal) {
	...
	[Obsolete]
	public void circle (float radius, int divisions,  Vector3 center,  Vector3 normal,  Vector3 tangent,
		 Vector3 binormal, float angleFrom, float angleTo) {
	...
	[Obsolete]
	public void ellipse (float width, float height, int divisions,  Vector3 center,  Vector3 normal, float angleFrom,
		float angleTo) {
	...
	[Obsolete]
	public void cylinder (float width, float height, float depth, int divisions, float angleFrom, float angleTo, bool close) {
