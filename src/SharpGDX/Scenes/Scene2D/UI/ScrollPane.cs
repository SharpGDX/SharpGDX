...
public class ScrollPane : WidgetGroup
{
    ...
    private class ScrollPaneInputListener : InputListener
    {
        ...
        public override bool touchDown(InputEvent @event, float x, float y, int pointer, int button)
        {
            ...
            if (_scrollPane.scrollBarTouch && _scrollPane._scrollX && _scrollPane.hScrollBounds.contains(x, y))
            {
                ...
                _scrollPane.setScrollX(_scrollPane.amountX +
                                       _scrollPane.actorArea.width * (x < _scrollPane.hKnobBounds.x ? -1 : 1));
                return true;
            }
            ...
            if (_scrollPane.scrollBarTouch && _scrollPane._scrollY && _scrollPane.vScrollBounds.contains(x, y))
            {
                ...
                _scrollPane.setScrollY(_scrollPane.amountY +
                                       _scrollPane.actorArea.height * (y < _scrollPane.vKnobBounds.y ? 1 : -1));
                return true;
            }
        ...
        public override void touchDragged(InputEvent @event, float x, float y, int pointer)
        {
            ...
            if (_scrollPane.touchScrollH)
            {
                ...
                scrollH = Math.Min(
                    _scrollPane.hScrollBounds.x + _scrollPane.hScrollBounds.width - _scrollPane.hKnobBounds.width,
                    scrollH);
            ...
            else if (_scrollPane.touchScrollV)
            {
                ...
                scrollV = Math.Min(
                    _scrollPane.vScrollBounds.y + _scrollPane.vScrollBounds.height - _scrollPane.vKnobBounds.height,
                    scrollV);
    ...
    public override void act(float delta)
    {
        ...
        if (smoothScrolling && flingTimer <= 0 && !panning && //
            ...
            ((!touchScrollH ||
              (_scrollX && maxX / (hScrollBounds.width - hKnobBounds.width) > actorArea.width * 0.1f)) && //
             (!touchScrollV ||
              (_scrollY && maxY / (vScrollBounds.height - vKnobBounds.height) > actorArea.height * 0.1f))) //
           )
        {
    ...
    public override void layout()
    {
        ...
        actorArea.Set(bgLeftWidth, bgBottomHeight, width - bgLeftWidth - bgRightWidth,
