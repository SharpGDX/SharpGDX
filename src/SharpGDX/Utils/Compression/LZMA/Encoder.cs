using System;
using SharpGDX.Utils.Compression.RangeCoder;
...
namespace SharpGDX.Utils.Compression.LZMA
{
	...
	{
		public static readonly int EMatchFinderTypeBT2 = 0;
		public static readonly int EMatchFinderTypeBT4 = 1;

		static readonly int kIfinityPrice = 0xFFFFFFF;
		...
		static readonly int kDefaultDictionaryLogSize = 22;
		static readonly int kNumFastBytesDefault = 0x20;

		class LiteralEncoder
		{
			internal class Encoder2
			{
				short[] m_Encoders = new short[0x300];

				public void Init()
				{
					RangeCoder.Encoder.InitBitModels(m_Encoders);
				}

				public void Encode(RangeCoder.Encoder rangeEncoder, byte symbol) // TODO: throws IOException
				{
					int context = 1;
					for (int i = 7; i >= 0; i--)
					{
				...
				public void EncodeMatched(RangeCoder.Encoder rangeEncoder, byte matchByte,
					byte symbol) // TODO: throws IOException
				{
					int context = 1;
					bool same = true;
					for (int i = 7; i >= 0; i--)
		...
		public static readonly int kNumLenSpecSymbols = Base.kNumLowLenSymbols + Base.kNumMidLenSymbols;
		...
		static readonly int kNumOpts = 1 << 12;
		...
		int ReadMatchDistances() // TODO: throws java.io.IOException
		{
			int lenRes = 0;
		...
		void MovePos(int num) // TODO: throws java.io.IOException 
		{
			if (num > 0)
			{
		...
		int GetOptimum(int position) // TODO: throws IOException 
		{
			...
			_optimum[1].Price = RangeCoder.Encoder
			...
			int matchPrice = RangeCoder.Encoder
			...
			while (true)
			{
				...
				matchPrice = curPrice + RangeCoder.Encoder
					...
					if (lenTest < numAvailableBytesFull)
					{
						...
						if (lenTest2 >= 2)
						{
							...
							int curAndLenCharPrice = repMatchPrice + GetRepPrice(repIndex, lenTest, state, posState)
							                                       + RangeCoder.Encoder
								                                       .GetPrice0(_isMatch[
									                                       (state2 << Base.kNumPosStatesBitsMax) +
									                                       posStateNext])
							...
							int nextMatchPrice = curAndLenCharPrice + RangeCoder.Encoder
									...
									int curAndLenCharPrice = curAndLenPrice
									                         + RangeCoder.Encoder
										                         .GetPrice0(_isMatch[
											                         ...
											                         posStateNext])
									                         + _literalEncoder.GetSubCoder(position + lenTest,
										                         _matchFinder.GetIndexByte(lenTest - 1 - 1)).GetPrice(
										                         true,
		...
		bool ChangePair(int smallDist, int bigDist)
		{
			int kDif = 7;
			return (smallDist < (1 << (32 - kDif)) && bigDist >= (smallDist << kDif));
		}

		void WriteEndMarker(int posState) // TODO: throws IOException 
		{
		...
		void Flush(int nowPos) // TODO: throws IOException 
		{
			ReleaseMFStream();
		...
		public void CodeOneBlock(long[] inSize, long[] outSize, bool[] finished) // TODO: throws IOException 
		{
			inSize[0] = 0;
			outSize[0] = 0;
			finished[0] = true;
		...
		public void Code(InputStream inStream, OutputStream outStream, long inSize, long outSize,
			ICodeProgress progress) // TODO: throws IOException
		{
			_needReleaseMFStream = false;
			try
			{
		...
		public static readonly int kPropSize = 5;
		byte[] properties = new byte[kPropSize];

		public void WriteCoderProperties(OutputStream outStream) // TODO: throws IOException 
		{
		...
		void FillDistancesPrices()
		{
			for (int i = Base.kStartPosModelIndex; i < Base.kNumFullDistances; i++)
			{
				...
				int baseVal = ((2 | (posSlot & 1)) << footerBits);
				tempPrices[i] =
					BitTreeEncoder.ReverseGetPrice(_posEncoders, baseVal - posSlot - 1, footerBits, i - baseVal);
			}
			...
			for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
			{
				...
				for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
					_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1)
					                                  - Base.kNumAlignBits) <<
					                                 RangeCoder.Encoder.kNumBitPriceShiftBits);
		...
		{
			if (lp < 0 || lp > Base.kNumLitPosStatesBitsEncodingMax || lc < 0 || lc > Base.kNumLitContextBitsMax ||
			    pb < 0
