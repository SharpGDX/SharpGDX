...
namespace SharpGDX.Shims
{
    ...
    : IComparable<Path>, IEnumerator<Path>//, Watchable
{
    /**
    ...
    public static Path of(String first, String[] more) {
        return DefaultFileSystem.getFileSystem().getPath(first, more);
    }
     ...
     * @since 11
     */
    // TODO: public static Path of(URI uri) {
     ...
     * @since 22
     */
    Path resolve(Path first, Path[] more) {
        Path result = resolve(first);
        foreach (Path p in more) {
            result = result.resolve(p);
        }
        return result;
    }
     ...
     * @see FileSystem#getPath
     */
    // TODO: Path resolveSibling(String other) {
     ...
     *          system property {@code user.dir}
     */
    // TODO: Path toRealPath(LinkOption... options) throws IOException;
     ...
     */
    File toFile() {
        if (getFileSystem() == DefaultFileSystem.getFileSystem()) {
     ...
     */
    IEnumerator<Path> GetEnumerator()
    {
        return new Enumerator(this);
    }

    private class Enumerator : IEnumerator<Path>
    {
    ...
    public Path Current
    {
        get
        {
            if (i < _path.getNameCount())
            {
                Path result = _path.getName(i);
                i++;
                return result;
            }
            else
