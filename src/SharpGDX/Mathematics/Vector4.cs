...
public class Vector4 :  IVector<Vector4> {
	...
	public readonly static Vector4 X = new Vector4(1, 0, 0, 0);
	public readonly static Vector4 Y = new Vector4(0, 1, 0, 0);
	public readonly static Vector4 Z = new Vector4(0, 0, 1, 0);
	public readonly static Vector4 W = new Vector4(0, 0, 0, 1);
	...
	public bool idt ( Vector4 vector) {
		return x == vector.x && y == vector.y && z == vector.z && w == vector.w;
	}

	/** @return The Euclidean distance between the two specified vectors */
	public static float dst ( float x1,  float y1,  float z1,  float w1,  float x2,  float y2,
		 float z2,  float w2) {
		 float a = x2 - x1;
		 float b = y2 - y1;
		 float c = z2 - z1;
	...
	public static float dst2 ( float x1,  float y1,  float z1,  float w1,  float x2,  float y2,
		 float z2,  float w2) {
		 float a = x2 - x1;
		 float b = y2 - y1;
		 float c = z2 - z1;
	...
	public bool isOnLine (Vector4 other, float epsilon) {
	...
	/** @return true if this vector is in line with the other vector (either in the same or the opposite direction) */
	public bool isOnLine (Vector4 other) {
	...
	public bool isCollinear (Vector4 other, float epsilon) {
		return isOnLine(other, epsilon) && hasSameDirection(other);
	}

	/** @return true if this vector is collinear with the other vector ({@link #isOnLine(Vector4)} &&
	 *         {@link #hasSameDirection(Vector4)}). */
	public bool isCollinear (Vector4 other) {
		return isOnLine(other) && hasSameDirection(other);
	}

	/** @return true if this vector is collinear with the other vector ({@link #isOnLine(Vector4, float)} &&
	 *         {@link #hasSameDirection(Vector4)}). */
	public bool isCollinearOpposite (Vector4 other, float epsilon) {
		return isOnLine(other, epsilon) && hasOppositeDirection(other);
	}

	/** @return true if this vector is collinear with the other vector ({@link #isOnLine(Vector4)} &&
	 *         {@link #hasSameDirection(Vector4)}). */
	public bool isCollinearOpposite (Vector4 other) {
	...
	public bool isPerpendicular (Vector4 vector, float epsilon) {
		return MathUtils.isZero(dot(vector), epsilon);
	}

	public bool hasSameDirection (Vector4 vector) {
		return dot(vector) > 0;
	}

	public bool hasOppositeDirection (Vector4 vector) {
		return dot(vector) < 0;
	}
	...
	public Vector4 fromString (String v) {
		...
		if (s0 != -1 && s1 != -1 && v[0] == '(' && v[v.Length - 1] == ')') {
			try {
				...
				float z = float.Parse(v.Substring(s1 + 1, s2));
				float w = float.Parse(v.Substring(s2 + 1, v.Length - 1));
	...
	public bool epsilonEquals ( Vector4 other, float epsilon) {
	...
	public bool epsilonEquals (float x, float y, float z, float w, float epsilon) {
	...
	public bool epsilonEquals ( Vector4 other) {
	 ...
	 * @param w w component of the other vector to compare
	 * @return true if the vectors are equal, otherwise false */
	public bool epsilonEquals (float x, float y, float z, float w) {
